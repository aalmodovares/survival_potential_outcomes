...
class SATEDVAEM2(Wrapper):
    def __init__(self, input_dim,
                 encoder_name, decoder_name, treatment_regressor_name, outcome_regressor_name, censoring_regressor_name,
                 latent_dim_t, latent_dim_c, latent_dim_y,
                 hidden_sizes_encoder, hidden_sizes_decoder, hidden_sizes_treatment_regressor, hidden_sizes_outcome_regressor, hidden_sizes_censoring_model,
                 outcome_dist, censoring_dist, num_treatments,
                 latent_dist, data_types,
                 loss_weights,
                 experiment, model_path,
                 dropout,
                 activation_encoder, activation_decoder, activation_treatment_regressor, activation_outcome_regressor, activation_censoring_model,
                 device,
                 weight_init_encoder=None, weight_init_decoder=None, weight_init_treatment_regressor=None, weight_init_outcome_regressor=None, weight_init_censoring_model=None,
                 save=True,
                 limit_variance_encoder =False, limit_variance_decoder=False, limit_variance_outcome=False, limit_variance_censoring=False,
                 min_std_outcome=None, max_std_outcome=None,
                 min_std_censoring=None, max_std_censoring=None,
                 fix_variance_encoder = False, fix_variance_decoder = False, fix_variance_outcome = False, fix_variance_censoring = False,
                 fixed_std_outcome = None, fixed_std_censoring = None,
                 limit_shape_encoder = False, limit_shape_decoder=False, limit_shape_outcome=False, limit_shape_censoring=False,
                 optim_config=None, wandblog=False, disable_tqdm=False):

        super(SATEDVAEM2, self).__init__('tedvae', device, experiment, model_path, save, optim_config, wandblog, disable_tqdm)
        ...
        self.hidden_dizes_censoring_model = hidden_sizes_censoring_model
        ...
        self.min_std_censoring = min_std_censoring
        self.max_std_censoring = max_std_censoring
        ...
        self.censoring_model = T_learner(network_name = censoring_regressor_name,
                                         input_dim = self.latent_dim_t + self.latent_dim_c,
                                         ...
                                         save = self.save,
                                         limit_variance = self.limit_variance_censoring,
                                         min_std = self.min_std_censoring,
                                         max_std = self.max_std_censoring,
        ...
        self.censoring_guide = TARNet(network_name = censoring_regressor_name,
                                      input_dim = self.latent_dim_t + self.latent_dim_c,
    ...
    def forward(self, x_dict):
        ...
        return {'q_z_t': q_z_t, 'q_z_c': q_z_c, 'q_z_y': q_z_y,
                ...
                'q_c_guide_list': q_c_guide_list, 'p_c_model_list': p_c_model_list}
    ...
    def loss(self, output, true_values):
        ...
        loss_disentanglement = - self.loss_weights['w_disentanglement_t'] *log_prob_q_t.mean() + self.loss_weights['w_disentanglement_y']*loss_q_y + self.loss_weights['w_disentanglement_c']*loss_q_c
        ...
        loss_model = - self.loss_weights['w_model_t'] * log_prob_p_t.mean() + self.loss_weights['w_model_y'] *loss_p_y + self.loss_weights['w_model_c'] * loss_p_c
    ...
    def predict(self, X_test, load_model:str | None  = 'best'):
        ...
        output_dict = {'propensity_score_pred': propensity_score_pred, 'y_dist_list': y_dist_list, 'c_dist_list': c_dist_list}
